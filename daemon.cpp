...
#else
#define   LOG_I(fmt, ...) printf("client_droid:" fmt "\n", ##__VA_ARGS__)
#define   LOG_E(fmt, ...) printf("client_droid:" fmt "\n", ##__VA_ARGS__)
#endif
...
#if 0
    string cmd = "logwrapper /system/bin/client_droid_internal";
    LOG_I("start to execute: %s\n", cmd.c_str());
    int ret = exec_cmd(cmd.c_str());
#else
    string cmd = "/system/bin/client_droid_internal 2>&1";
    LOG_I("start to execute: %s\n", cmd.c_str());
